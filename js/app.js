(() => {
    ...
    function heroImageAnimation(gsap) {
    ...
    function changeWordInString() {
        ...
        if (heroWorker && typeArrey) for (let i = 0; i < typeArrey.children.length; i++) {
            const element = typeArrey.children[i];
            element.style.textShadow = "#ffffff 1px 0 2px";
            if (i >= 0 && i <= 4 || i >= 9 && i <= 11 || i >= typeArrey.children.length - 2 && i <= typeArrey.children.length) element.style.fontWeight = 800;
            if (i === typeArrey.children.length - 2) element.style.color = "#F3B95F";
            if (i === typeArrey.children.length - 1) element.style.color = "#0B60B0";
            if (i === typeArrey.children.length - 4 || i === typeArrey.children.length - 5) element.style.textShadow = "#ffffff 1px 0 2px";
        }
    }
    function heroTextAnimation(gsap) {
        var circleType = new CircleType(document.getElementById("subTitleAnimated"));
        circleType.radius(384).dir(-1);
        new SplitType("[titleAnimated]", {
            types: "lines, words, chars",
            tagName: "span"
        });
        new SplitType("[subTitleAnimated]", {
            types: "lines, words, chars",
            tagName: "span"
        });
    ...
    function briefsTextAnimation(gsap) {
        ...
        if (window.innerWidth > 1352.98) tl.fromTo(briefsItem[0], {
            x: `-300%`,
            opacity: 0
        }, {
            x: "0%",
            ...
            opacity: 1,
            duration: .3
        }); else if (window.innerWidth > 767.98 && window.innerWidth <= 1352.98) tl.fromTo(briefsItem[0], {
            x: `-300%`,
            y: `-300%`,
            opacity: 0
        }, {
            ...
            opacity: 1,
            duration: .3
        }); else if (window.innerWidth <= 767.98) tl.fromTo(briefsItem[0], {
            x: `-300%`,
            opacity: 0
        }, {
            x: "0%",
...
*/
    ...
    function Observer_defineProperties(target, props) {
        for (var i = 0; i < props.length; i++) {
            var descriptor = props[i];
            descriptor.enumerable = descriptor.enumerable || false;
            descriptor.configurable = true;
    ...
    function Observer_createClass(Constructor, protoProps, staticProps) {
        if (protoProps) Observer_defineProperties(Constructor.prototype, protoProps);
        if (staticProps) Observer_defineProperties(Constructor, staticProps);
        return Constructor;
    }
    /*!
 * Observer 3.12.5
...
*/
    ...
    document.addEventListener("DOMContentLoaded", (function(e) {
        heroImageAnimation(gsapWithCSS);
        briefsCount(gsapWithCSS);
        buttonAnim(gsapWithCSS);
    }));
    window["FLS"] = true;
    isWebp();
    addLoadedClass();
